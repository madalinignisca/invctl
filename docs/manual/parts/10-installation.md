...
A custom field's value is never written to `change_log` as text: a plain
change counter is folded into the audited entry instead (which field, how
many times it has changed), so a change still shows up as a diff without the
value itself reaching the log. There is no key involved and nothing to
configure — see `docs/AUDIT.md` for the mechanism and why it replaced an
earlier keyed digest.
