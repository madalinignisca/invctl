...
// CreateAssetInProject creates a NEW asset and links it to projectID in the
// SAME transaction -- the WP-G1 Task 14 shape (docs/rbac-design.md §4) a
// project owner can reach because the project is a path parameter rather
// than a form field: the asset is new by construction of the route, so
// there is nothing a caller could seize by naming an existing id on the
// form (see domain.scopedPermit.Covers's carve-out comment for the other
// half of this argument -- Covers admits project_asset only for a project
// this permit holds).
//
// Environment membership is deliberately not a parameter here, unlike
// CreateAsset: this route exists so a project owner can add their own box
// without an administrator, and environment assignment is estate-adjacent
// enough (it drives observed-state scoping, internal/store/observed.go) to
// stay a follow-up edit through UpdateAsset rather than something this
// narrower route also has to arbitrate.
//
// The project_asset INSERT is written out here rather than calling
// LinkProjectAsset (internal/store/projects.go): that method's UPSERT
...
func (s *SQLStore) CreateAssetInProject(ctx context.Context, p domain.Permit, projectID string, a *domain.Asset) error {
	// The row the INSERT just wrote is version 1 (the column default). See
	// CreateAsset's identical comment.
	a.RowVersion = 1
	if err := a.Validate(); err != nil {
		return err
	}
	// THE SECURITY CHECK, on the CALLER'S OWN permit, before anything else
	// runs. domain.PermitHoldsProject asks p -- not a permit this method is
	// about to construct -- whether it genuinely holds projectID; a project
	// owner assigned elsewhere gets ErrForbidden here, before a transaction
	// opens or a rack is even queried.
	if !domain.PermitHoldsProject(p, projectID) {
		return fmt.Errorf("creating an asset in project %s: %w", projectID, domain.ErrForbidden)
	}
	// Rack space before the transaction opens, for the same reason and with
	// the same accepted race window as CreateAsset -- see that method's
	// comment.
	if err := s.requireFreeRackSpace(ctx, a); err != nil {
		return err
	}
...
// insertProjectAssetLink writes the `owns` link row for an asset just
// created in this same transaction. Never ON CONFLICT: the asset id was
// generated by s.insertAsset a moment ago, so no project_asset row for it
// can already exist, and a plain INSERT that fails loudly if that
// assumption is ever wrong is a stronger statement than an upsert that
// would paper over it.
func (s *SQLStore) insertProjectAssetLink(ctx context.Context, t *tx, projectID, assetID string) error {
	now := domain.FormatTime(s.now())
	link := &domain.ProjectAssetLink{
		ProjectID: projectID, AssetID: assetID, Relation: domain.ProjectOwns,
	...
	_, err := t.exec(ctx, `
		...
		VALUES (?, ?, ?, ?, ?, ?, ?)`,
		link.ProjectID, link.AssetID, link.Relation, link.Note, link.Lifecycle, link.CreatedAt, link.UpdatedAt)
	if err != nil {
		return translateWriteErr(err, "linking new asset to project")
	}
	return t.logCreate(ctx, "project_asset", link.ProjectID+"/"+link.AssetID, link)
}
