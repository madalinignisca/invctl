...
-- v1.54.0, SQLite 3.53.3): ADD COLUMN then ADD CONSTRAINT ... CHECK both
-- succeed, the constraint validates existing rows, a malformed date is
-- rejected, and the resulting DDL carries the constraint with no table
-- rebuild. 00011 recorded the same measurement and its own header says to
-- re-take it rather than trust the line; this is that.
--
