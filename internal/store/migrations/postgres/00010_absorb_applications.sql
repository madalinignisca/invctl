...
--
-- NO REBUILD IS NEEDED. PostgreSQL drops a column and the index that covers
-- it together, so the explicit DROP INDEX the SQLite half needs first is
-- harmless here and is kept only so the two files read the same. (SQLite
-- refuses the DROP COLUMN outright while the index exists -- measured, see the
-- sqlite half.)
--
