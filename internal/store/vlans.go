...
// ---------- the backfill from the loose integer ----------

// BackfillPrefixVLANs turns each distinct legacy prefix.vlan_id into a real
// VLAN and points the prefix at it.
//
// WHY THIS IS GO AND NOT PART OF MIGRATION 00031. Every row it creates needs a
// UUIDv7 id and an RFC3339 timestamp, and this codebase generates both in Go
// and never in SQL -- that rule is why the ids sort by time and why no row
// depends on a database clock. A migration cannot call Go, so the migration
// widens the schema and this fills it: expand, backfill, contract.
//
// IDEMPOTENT, and it has to be, because it runs on every start. A prefix that
// already points at a VLAN is left alone, and a VID that already has a VLAN is
// reused rather than duplicated -- so two prefixes on VLAN 30, one v4 and one
// v6, end up on the SAME broadcast domain, which is the fact the integer could
// never express and the whole reason for the model.
//
// Returns how many VLANs it created, so a deployment can see it happen once
// and never again.
func (s *SQLStore) BackfillPrefixVLANs(ctx context.Context, actor domain.Actor) (int, error) {
	var pending []struct {
		ID     string `db:"id"`
		VLANID int    `db:"vlan_id"`
	}
	err := s.read(ctx, &pending, `
		SELECT id, vlan_id FROM prefix
		WHERE vlan_id IS NOT NULL AND vlan_ref_id IS NULL
		ORDER BY vlan_id`)
	if err != nil {
		return 0, fmt.Errorf("finding prefixes with a legacy VLAN id: %w", err)
	}
	if len(pending) == 0 {
		return 0, nil
	}

	// What is already there, so a re-run adopts rather than duplicates.
	var existing []struct {
		ID  string `db:"id"`
		VID int    `db:"vid"`
	}
	err = s.read(ctx, &existing, `
		SELECT id, vid FROM vlan WHERE group_id IS NULL AND lifecycle <> 'retired'`)
	if err != nil {
		return 0, fmt.Errorf("reading existing vlans: %w", err)
	}
	byVID := map[int]string{}
	...
	for _, p := range pending {
		id, ok := byVID[p.VLANID]
		if !ok {
			// Named for the number, because that is genuinely all the old column
			// said. A name somebody has to correct is better than one this
			// invents and nobody questions.
			v, err := domain.NewVLAN(NewID(), p.VLANID, fmt.Sprintf("VLAN %d", p.VLANID), nil)
			if err != nil {
				return created, fmt.Errorf("building vlan %d: %w", p.VLANID, err)
			}
			v.Description = strPtrVLAN("created from prefix.vlan_id when VLANs became a model")
			if err := s.CreateVLAN(ctx, actor, v); err != nil {
				return created, fmt.Errorf("creating vlan %d: %w", p.VLANID, err)
			}
			byVID[p.VLANID] = v.ID
			id = v.ID
			created++
		}
		// The link is a declared change on the prefix and takes its own audit
		// entry, like any other -- a backfill is still somebody's decision, it
		// is just one taken by whoever chose to deploy this version.
		...
		after := *before
		after.VLANRefID = &id
		if err := s.linkPrefixVLAN(ctx, actor, before, &after); err != nil {
			return created, err
		}
	}
	return created, nil
...
func (s *SQLStore) linkPrefixVLAN(ctx context.Context, actor domain.Actor,
	before, after *domain.Prefix) error {

	at := domain.FormatTime(s.now())
	after.UpdatedAt = &at
	return s.write(ctx, actor, func(t *tx) error {
		res, err := t.exec(ctx, `
			UPDATE prefix SET vlan_ref_id = ?, updated_at = ?,
			                  row_version = row_version + 1
			WHERE id = ? AND row_version = ?`,
			after.VLANRefID, at, after.ID, before.RowVersion)
		if err != nil {
			return translateWriteErr(err, "linking prefix to vlan")
		}
		if err := requireVersion(res, "prefix", after.ID, &after.RowVersion); err != nil {
			return err
		}
		return t.logUpdate(ctx, "prefix", after.ID, before, after)
	})
}
