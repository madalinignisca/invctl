...
func (a *App) CostAddToAsset(w http.ResponseWriter, r *http.Request) {
	...
	if err == nil {
		err = a.Store.AddAssetCost(r.Context(), a.permit(r), id, c)
	}
...
func (a *App) CostConsumersOnAsset(w http.ResponseWriter, r *http.Request) {
	...
	err := a.Store.SetCostConsumers(r.Context(), a.permit(r), costID, r.PostForm["consumers"])
	if err != nil {
...
func (a *App) CostAddToService(w http.ResponseWriter, r *http.Request) {
	...
	c, err := costFromForm(r, a.Store.Now())
	if err == nil {
		err = a.Store.AddServiceCost(r.Context(), a.permit(r), id, c)
	}
...
func (a *App) CostAddToProject(w http.ResponseWriter, r *http.Request) {
	...
	c, err := costFromForm(r, a.Store.Now())
	if err == nil {
		err = a.Store.AddProjectCost(r.Context(), a.permit(r), id, c)
	}
...
func (a *App) editCost(r *http.Request, costID string,
	...
	return update(r.Context(), a.permit(r), &updated)
}

// CostRetireOnAsset soft-deletes a line on an asset.
func (a *App) CostRetireOnAsset(w http.ResponseWriter, r *http.Request) {
	err := a.Store.RetireAssetCost(r.Context(), a.permit(r), r.PathValue("id"), r.PathValue("costID"))
...
func (a *App) CostRetireOnService(w http.ResponseWriter, r *http.Request) {
	err := a.Store.RetireServiceCost(r.Context(), a.permit(r), r.PathValue("id"), r.PathValue("costID"))
...
func (a *App) CostRetireOnProject(w http.ResponseWriter, r *http.Request) {
	err := a.Store.RetireProjectCost(r.Context(), a.permit(r), r.PathValue("id"), r.PathValue("costID"))
...
func (a *App) CostRepriceOnAsset(w http.ResponseWriter, r *http.Request) {
	...
	if err == nil {
		_, err = a.Store.RepriceAssetCost(r.Context(), a.permit(r), id, spec)
	}
	a.afterCostWrite(w, r, err, "/assets/"+id)
}
...
func (a *App) CostRepriceOnService(w http.ResponseWriter, r *http.Request) {
	...
	spec, err := repriceFromForm(r)
	if err == nil {
		_, err = a.Store.RepriceServiceCost(r.Context(), a.permit(r), id, spec)
	}
	a.afterCostWrite(w, r, err, "/services/"+id)
}
...
func (a *App) CostRepriceOnProject(w http.ResponseWriter, r *http.Request) {
	...
	spec, err := repriceFromForm(r)
	if err == nil {
		_, err = a.Store.RepriceProjectCost(r.Context(), a.permit(r), id, spec)
	}
	a.afterCostWrite(w, r, err, "/projects/"+id)
}
...
func (a *App) CostRepriceOnCircuit(w http.ResponseWriter, r *http.Request) {
	...
	spec, err := repriceFromForm(r)
	if err == nil {
		_, err = a.Store.RepriceCircuitCost(r.Context(), a.permit(r), id, spec)
	}
...
func (a *App) CostAddToCircuit(w http.ResponseWriter, r *http.Request) {
	...
	c, err := costFromForm(r, a.Store.Now())
	if err == nil {
		err = a.Store.AddCircuitCost(r.Context(), a.permit(r), id, c)
	}
	a.afterCostWrite(w, r, err, "/circuits/"+id)
}

func (a *App) CostEditOnCircuit(w http.ResponseWriter, r *http.Request) {
	id := r.PathValue("id")
	costID := r.PathValue("costID")
	err := a.editCost(r, costID, a.Store.GetCircuitCost, a.Store.UpdateCircuitCost, id)
	a.afterCostEdit(w, r, err, "/circuits/"+id, costID)
}

func (a *App) CostRetireOnCircuit(w http.ResponseWriter, r *http.Request) {
	id := r.PathValue("id")
	err := a.Store.RetireCircuitCost(r.Context(), a.permit(r), id, r.PathValue("costID"))
	a.afterCostWrite(w, r, err, "/circuits/"+id)
}
