...
- Two `DELETE FROM` statements remove a *fact* rather than replacing a set, and both are store code, never handler code. The admin-invoked prune of `observed_transition` (telemetry, never a fact about the estate — see `docs/AUDIT.md` rule 10). And `ScrubUser`'s deletion of a scrubbed person's `saved_view` rows: a saved view is one person's shortcut, not estate history, so once its owner is erased it belongs to nobody and serves nothing, and keeping it is retaining personal data with no purpose (`docs/AUDIT.md` rule 16). Both are narrow, named exceptions to soft-delete, not a precedent — a third one needs the same reasoning and the same sign-off, not an assumption that the door is now open.
