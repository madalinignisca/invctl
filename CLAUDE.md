...
- Three `DELETE FROM` statements remove a row rather than replacing a set, and all are store code, never handler code. The admin-invoked prunes of `observed_transition` and `unmatched_observation` (both telemetry, neither a fact about the estate — see `docs/AUDIT.md` rule 10). And `ScrubUser`'s deletion of a scrubbed person's `saved_view` rows: a saved view is one person's shortcut, not estate history, so once its owner is erased it belongs to nobody and serves nothing, and keeping it is retaining personal data with no purpose (`docs/AUDIT.md` rule 16). All three are narrow, named exceptions to soft-delete, not a precedent — a fourth needs the same reasoning and the same sign-off, not an assumption that the door is now open. `TestTheOnlyFactDeletingStatementIsThePrune` enforces this per *table*, not per file: a delete of an undeclared table fails even in a file that is already allowlisted. That tightening is what found `unmatched_observation` missing from this list in the first place.
